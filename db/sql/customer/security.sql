...
SELECT AddMemberToGroup(CreateUser('daemon', 'daemon', 'Демон', null, null, 'Пользователь для API', false, true, GetArea('root')), 1000);
...
SELECT AddMemberToGroup(CreateUser('apibot', 'apibot', 'Системная служба API', null, null, 'API клиент', false, true, GetArea('default')), 1000);
SELECT CreateUser('mailbot', 'mailbot', 'Почтовый клиент', null, null, 'Почтовый клиент', false, true, GetArea('default'));
