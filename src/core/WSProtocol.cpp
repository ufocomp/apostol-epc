...
namespace Apostol {
    ...
    namespace WSProtocol {
        ...
        CSession *CSession::FindOfConnection(CHTTPServerConnection *AConnection, const CString &Name) {
            int Index = AConnection->Data().IndexOfName(Name);
            if (Index == -1)
                throw Delphi::Exception::ExceptionFrm("Not found \"%s\" in connection.", Name.c_str());

            auto Session = dynamic_cast<CSession *> (AConnection->Data().Objects(Index));
            if (Session == nullptr)
